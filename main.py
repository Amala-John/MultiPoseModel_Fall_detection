...
# ip_addresses = {}
# file_streams = {}
# model_path = "D:/Mutipose/Real-Time-Fall-Detection/ok.pt"
...
# Define model path and initialize frame queue
model_path = "D:/Mutipose/Real-Time-Fall-Detection/ok.pt"
frame_queue = queue.Queue(maxsize=10)

# Initialize VideoProcessor
video_processor = VideoProcessor(model_path, frame_queue)

def main():
    # Open the default camera (0 is the device ID for the primary camera)
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        print("Error: Could not open the device camera.")
        return
